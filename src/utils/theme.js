export const cardCtaStyles =
  "bg-b2 py-1 flex-grow rounded-full text-sm font-medium text-white text-center border boder-white hover:bg-white hover:text-b2 transition";

  export const modalStyles = "bg-white max-w-sm rounded-lg p-6";
  export const modalHeaderStyles =
    "text-2xl font-bold text-center text-b2 mb-4";

  export const modalInputStyles =
    "outline-none bg-b2 bg-opacity-10 rounded px-4 py-2 w-full mb-6";

  export const modalPrimaryCtaStyles =
    "bg-b2 rounded-full px-8 py-2 text-white text-center font-medium";

  export const modalSecCtaStyles =
    "bg-white rounded-full px-8 py-2 text-b2 text-center font-medium border border-b2";